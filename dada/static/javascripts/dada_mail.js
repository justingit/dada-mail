...
$(document).ready(function() {
	...
	if($('#schedule_datetime').length) { 
		...
		if ($("#send_email_screen").length || $("#send_url_email").length) {
			...
			$("body").on("click", ".toggle_schedule_options", function(event) {
				toggle_schedule_options(); 
			}); 
			
			$("body").on("change", ".schedule_fields", function(event) {
				//preview_mass_mail_schedules(); 
			}); 
			$("body").on("click", ".schedule_fields", function(event) {
				preview_mass_mail_schedules(); 
			}); 
			...
			$("body").on("click", "#create_schedule", function(event) {
				...
				//popup_schedules_fields_to_schedule_fields(); 
...
function toggle_schedule_options() {
	...
	if ($("#popup_schedule_type_single").prop("checked") === true) {

		if ($('#popup_schedule_type_single_options').is(':hidden')) {
			$('#popup_schedule_type_single_options').show('blind');
		}
		
		if ($('#popup_schedule_type_recurring_options').is(':visible')) {
			$('#popup_schedule_type_recurring_options').hide('blind');
		}
		
	} else if ($("#popup_schedule_type_recurring").prop("checked") === true) {
		
		if ($('#popup_schedule_type_recurring_options').is(':hidden')) {
			$('#popup_schedule_type_recurring_options').show('blind');
		}
		
		if ($('#popup_schedule_type_single_options').is(':visible')) {
			$('#popup_schedule_type_single_options').hide('blind');
		}
	
	}
}

function preview_mass_mail_schedules() { 
	
	$("#preview_mass_mail_schedules_results").hide().html('<p>Loading...</p>').show('fade');
	
	//popup_schedules_fields_to_schedule_fields();
	
	//alert($('.schedule_fields').serialize()); 
	var request = $.ajax({
		url: $("#s_program_url").val(),
		type: "POST",
		cache: false,
		data: $('.schedule_fields').serialize() + '&flavor=preview_mass_mail_schedules',
		dataType: "html"
	});
	request.done(function(content) {
		$("#preview_mass_mail_schedules_results").hide("fade", function() {
			$("#preview_mass_mail_schedules_results").html(content);
			$("#preview_mass_mail_schedules_results").show('fade');
		});
	});

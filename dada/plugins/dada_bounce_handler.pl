...
sub cl_main {
    ...
    if ( $help == 1 ) {
        ...
        my $bh = BounceHandler->new($Plugin_Config);
...
sub _init {
	...
	To do: Replace PLugin_Config with $self->config-{whatever...}
	And then move Rules into the 2nd obj
...
sub test_bounces {
    ...
    if ( $test_type eq 'pop3' ) {
        ...
        @files_to_test = $self->dir_list($test);
    }
    elsif ( -f $test_type ) {
...
sub parse_bounce {
    ...
    my $bp = BounceHandlerMessageParser->new;
...
sub bounce_from_me {
    ...
    $bh_header eq $Plugin_Config->{Plugin_Name} ? return 1 : return 0;
}
...
sub save_scores {
    ...
    if ( keys %$score ) {
        ...
        for my $d_list (@delete_list) {
            ...
            my $removal_list =
              $bsk->removal_list( $Plugin_Config->{Score_Threshold} );
...
sub add_to_score {
    ...
    return
"Email, '$email', on list: $list -  adding  $action to total score. Will remove after score reaches, $Plugin_Config->{Score_Threshold}\n";
...
package BounceHandlerMessageParser;

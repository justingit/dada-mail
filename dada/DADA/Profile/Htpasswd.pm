...
sub validate_protected_dir {
    ...
    if (   !exists( $args->{-fields}->{-name} )
        || !defined( $args->{-fields}->{-name} )
        || length( $args->{-fields}->{-name} ) <= 0 )
    {
        $errors->{missing_name} = 1;
        $status = 0;
    }
    if (   !exists( $args->{-fields}->{-url} )
        || !defined( $args->{-fields}->{-url} )
        || length( $args->{-fields}->{-url} ) <= 0 )
    {
    ...
    if (   !exists( $args->{-fields}->{-path} )
        || !defined( $args->{-fields}->{-path} )
        || length( $args->{-fields}->{-path} ) <= 0 )
    {
...
sub get_all_ids {
    ...
    my $query =
        'SELECT id FROM '
      . $self->{sql_params}->{password_protect_directories_table}
      . ' WHERE list = ? GROUP BY id ORDER BY id DESC;';
...
sub write_htaccess {
    ...
    if ( !-d make_safer( $entry->{path} ) ) {
        carp "Directory, '"
          . $entry->{path}
          . "' does not exist, skipping creation of .htaccess file.";
        return 0;
    }

    my $custom_error_page = undef;
    if ( $entry->{use_custom_error_page} == 1
        && defined( $entry->{custom_error_page} ) )
    {
    ...
    if ( -e $entry->{path} . '/' . '.htaccess' ) {
        $htaccess_content =
          $self->grab_htaccess_content( { -path => $entry->{path} } );
    }

    my $open_pat =
      quotemeta( '# Begin '
          . $DADA::Config::PROGRAM_NAME
          . ' Password Protect Directives' );
    my $close_pat =
      quotemeta( '# End '
          . $DADA::Config::PROGRAM_NAME
          . ' Password Protect Directives' );
    ...
    my $data = DADA::Template::Widgets::screen(
        {
            -screen =>
              'plugins/password_protect_directories/htaccess_file.tmpl',
            -vars => {
                name              => $entry->{name},
                path              => $entry->{path},
...
sub write_htpasswd {
    ...
    if ( !-d make_safer( $entry->{path} ) ) {
        carp "Directory, '"
          . $entry->{path}
          . "' does not exist, skipping creation of .htpasswd file.";
        return 0;
    }
    ...
    while ( my ( $email, $password ) = $sth->fetchrow_array ) {
        ...
        if ( $entry->{always_use_default_password} == 1 ) {

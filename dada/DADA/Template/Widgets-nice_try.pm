...
$args->{-engine} = 'html_template';
	...
	if($args->{-expr}){ 
	 ...
	 $template = HTML::Template->new(%Global_Template_Options, 
		...
		if($args->{-engine} eq 'html_template_compiled'){ 


			warn 'HTML::Template::Pro';
			require HTML::Template::Pro;
			$template = HTML::Template::Pro->new(%Global_Template_Options, 
										scalarref => $args->{-data}, 
	                                       ($args->{-dada_pseudo_tag_filter} == 1) ?
	                                       (
	                                                       filter => [ 
...
=cut
			...
			warn 'HTML::Template::Compiled::Classic';
			require Digest::MD5; 
			my $digest = Digest::MD5::md5_hex($args->{-data});
			my $tmp_file_name = $DADA::Config::TMP . '/' . $digest . '.tmp';
			if(-e $tmp_file_name){ 
				# ...
			}
			else { 
				open my $tmp_file, '>', DADA::App::Guts::make_safer($tmp_file_name) or die $!; 
				print   $tmp_file ${$args->{-data}}; 
				close   $tmp_file; 
			}
					
			$template = HTML::Template::Compiled::Classic->new(
			
				file_cache      => 1, 
				file_cache_dir  => $DADA::Config::TMP . '/template_cache', 
				cache           => 1, 
				#scalarref => $args->{-data}, 

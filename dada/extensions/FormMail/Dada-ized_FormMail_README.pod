...
=head1 OBTAINING Dada-ized FormMail

Dada-ized FormMail is located in the, I<dada/extensions/FormMail> directory of the main Dada Mail distribution, under the name, B<FormMail.pl> 
...
=item Dada-ized_FormMail_README.pod
...
The envelope sender address to use for all emails
...
If this is set to 1 then rather than returning the
...
=head2 $double_spacing 

If this is set to 1 (as it is by default) then a blank
...
If this is set to 1 then the content of any long text
...
If this is set to 0 ( or if $emulate_matts_code is set
...
B<Dada-ized FormMail Note:> 

By default, this script is located in the, I<dada/extensions/FormMail> directory

Uploading the B<FormMail> directory into your, I<cgi-bin/dada/extensions> direction on the server will make this script work beautifully. 
...
=head1 Dada-ized Specific FORM CONFIGURATION

Along with the default form configurations, the Dada-ized FormMail allows you to specify if you'd like to subscribe, or send a confirmation for subscription to the email address located in the B<email> form value. 

Do note, that some of these form fields have changed in the 2.10.11 version of Dada Mail: 

=head2 email

As well as the default behavior that the B<email> field has in FormMail, the Dada-ized FormMail will also use the value of the B<email> field for subscriptions or subscription confirmations.

=head2 dada_mail_list

Holds the B<listshortname> of the Dada Mail list you'd like to use

example:

        <input type='hidden' name='dada_mail_list' value='justin'> 

=head2 dada_mail_subscribe_email

Set to either, B<1> or, B<yes> if you'd like the email address submitted in the form to be subscribed to your Dada Mail list. 
...
Don't use the B<dada_mail_confirm_email> and B<dada_mail_subscribe_email> in the same form, most likely the,  B<dada_mail_subscribe_email> field value will be used. Most likely. 

=head1 COMMON PROBLEMS

=head2 Confusion over the qw operator
...
=head2 Sendmail switches removed

...
Dada Mail's outgoing SMTP library have been updated, which should provide
...
If something does go wrong, more information will be returned to help you get to the root 
of the problem. 

=head3 Upgraded POP3 Libraries

Dada Mail's incoming POP3 library have also been updated, which should provide
successful connections to a wider variety of POP3 servers and authentication 
flavors. Of note: 
...
and if you would like to explicitly verify the SSL Certificate.

Both SMTP and POP3 B<SSL/TLS> options require the C<IO::Socket::SSL> module to be installed. 
This can be done separately, or
by installing the C<Bundle::DadaMail> module. 

...
Visit Dada Mail (http://example.com/cgi-bin/dada/mail.cgi), with the following query string: 
...
C<%SQL_PARAMS> holds additional paramaters and login credentials for your SQL backend. 
...
=head1 $WYSIWYG_EDITOR_OPTIONS, $FILE_BROWSER_OPTIONS
...
$USER_TEMPLATE should hold the absolute server path to the template used for everything
other than the List Control Panel. 
...
=head1 $MONITOR_MAILOUTS_AFTER_EVERY_EXECUTION 
...
=head1 $ENFORCE_CLOSED_LOOP_OPT_IN
...
=head1 Multiple Mailing List Sending
...
=head1 $SCREEN_CACHE - Caching HTML Screens
...
=head1 $HIDDEN_SUBSCRIBER_FIELDS_PREFIX
...
=head2 $UNSUBSCRIPTION_REQUEST_MESSAGE
...
=head2 $HTML_UNSUBSCRIPTION_REQUEST_MESSAGE

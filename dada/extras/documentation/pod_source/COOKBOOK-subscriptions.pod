...
B<Optional>: B<fields> holds the Subscriber Profile Fields you would like to pass. 
...
Here's an example, with two Subscriber Profile Fields, C<first_name> and C<last_name>: 
...
To use the jQuery plugin, you'll first need to call the plugin javascript, along with other .js files it needs, in the head of your document: 

	<script src="http://example.com/static/javascripts/jquery-1.8.2.min.js"></script>
	<script src="http://example.com/static/javascripts/jquery.dadamail.js"></script>
	<script src="http://example.com/static/javascripts/dada_mail.user.js"></script>

	<link rel="stylesheet" href="http://example.com/static/css/colorbox/colorbox.css">
	<script src="http://example.com/static/javascripts/colorbox/jquery.colorbox-min.js"></script>
...
This will place a subscription form in a div with the id of, B<mydiv>

=head3 Initializtion Paramaters
...
B<modal> (default: B<1>) allows you to set if the results of requests submitted from the form are shown in a modal window, or if results are shown in the entire browser window, by redirecting directly to Dada Mail. 

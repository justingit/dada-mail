...
=head3 Enhanced Cross-Site Request Forgery Prevention (CSRF)

Here's an overview of CSRF: 

L<https://owasp.org/www-community/attacks/csrf>

Prior to v11.16.0, Dada Mail didn't have any protection against CSRF while logged into the list control panel, so if this is a concern for you, B<we absolutely suggest an upgrade> to prevent this vector of attack to v11.16.0 (and later versions). 
...
We also suggest running the site you have Dada Mail installed on run under B<https> as well as enable B<Google reCAPTCHA> (v2 or v3 are both fine).

Although we know of no known CSRF exploits that have happened in the wild, this vulnerability has been confirmed. 

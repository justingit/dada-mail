...
Image Resizing now available for all mailing list messages 

Email resizing was available in Dada Mail, but only for images that were dragged and dropped into the rich text editor.

Now, all images that are embedded in mailing list messages can now be resized. This includes messages sent to a mailing list via Bridge. 
Look for related options in the list control panel under, B<Mass Mailing: Options: Images and Attachments>: 
...
rather than merely linking to the images. The problem is that the more images you embed, the larger the total size of your message. 
Large messages will take longer to send, and longer for your subscribers to to download/view. 

=head3  Resize embedded images 

Check this option to resize embdedded images (default: checked). Resizing images can help keep message size low. There's no reason to embed a two meg image, but it's not always obvious how large an image is to some users. Let Dada Mail deal with that. 

=head4 Image Width Limit (pixels): 

This is where you can set the image width limit. The default is, B<580 pixels>, which is how wide the including mass mailing layouts are that come with Dada Mail. Any image wider will just be responsively shrunk anyways, so there's just no reason to use an image that's wider. 

=head3 Resize drag-and-drop images 

(default: checked). Check this option to resize drag and dropped images, as soon as they're uploaded. Images added to your message in any other way will be resized (if needed) when a message is sent out and you won't see the resized image in the rich text editor. 
...
You can install any (or all!) of these modules separately, or just install/upgrade the, B<Bundle::DadaMail> module. 
...
so you're not sending out enormous messages to thousands of people. 

The default mass mailing size limit is 5 megabytes, which is very roomy.
Checks on message limits are enabled by default. 

=head3 Mass Mailing Schedules Enhancements

More love have been giving to the mass mailing schedules. 
...
For example, you may not set a schedule to send your message on only the first Monday of each month. 
...
=head3 Mass Mailing from RSS/Atom feed enahncements 
...
Setting a time zone shown in the list control panel will be added in a future releae, but for now, the server's time zone is now exposed to the user via the UI to help them understand what time of day the app is running (and how that compares to local time)

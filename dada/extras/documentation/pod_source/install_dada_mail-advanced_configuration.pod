...
(Note: Magically creating a template requires that you have the C<HTML::Tree> CPAN Module installed)
...
Since you'll probably be pulling the source of the template from a different location than where Dada Mail is installed, you'll probably need to set a base href tag to the beginning of the HTML. Check this option, then fill out what the base href URL should be, below. 
...
=head4 Include a Copy of jQuery (v1.11.3)  

Keep this option checked, if your site's design doesn't use jQuery, but uncheck it, if a jQuery library is already being utilized. jQuery is used in almost every Wordpress installation, so if you're basing your user template off of a Wordpress design, uncheck this! 

=head4 Include a Copy of jQuery UI (v1.9.1) 

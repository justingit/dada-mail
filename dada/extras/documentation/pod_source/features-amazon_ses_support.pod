...
When you use Dada Mail with SES, Dada Mail I<will> set the C<Return-Path> header explicitly, which it wouldn't for sendmail or SMTP. For sendmail, it'll set the C<-f> flag to your List Administrator Address, SMTP it will set the Sender to your List Administrator Address - both of which will set the C<Return-Path>, but by the MTA itself.

Dada Mail's included bounce handler has rules specifically for Amazon SES. It's not quite clear if you need to authenticate the email address that the bounce handler uses with Amazon or not. Watch this thread: 

L<https://forums.aws.amazon.com/thread.jspa?threadID=80961>

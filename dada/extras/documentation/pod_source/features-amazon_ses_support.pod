...
Dada Mail can utilize Amazon SES, both by using the Amazon SES API, as well as using the Amazon SES SMTP Interface.
...
During installation of Dada Mail, you'll need to chose the  B<AWS Region/Endpoint>, and place in your B<AWS Access Key ID> and B<AWS Secret Access Key>.

That's it. 

Setting up things on the Amazon AWS side is a little more involved, which we will walkthough below. 

=head2 Obtaining your AWS Access Key ID and AWS Secret Access Key

There's a few ways to create the necessary Users/Groups in the Amazon AWS system, that will then give you the necessary keys you need. Below, we describe a simple way that should give you for a secure and specific user, just for SES. 
...
You'll then want to make a new IAM User at: 
...
Under, B<Enter User Names:> enter a username. For this walkthrough, I'm going to enter B<DadaDemo>. 
...
and copy the, B<Access Key ID> and, B<Secret Access Key>. 
...
Click on the user you just created (B<DadaDemo>).
...
You should then see the user's details. Click on the B<Permissions> tab. 
...
In that tab, click on the button labeled, B<Attach Policy>.

You should now see a table of available policies. Scroll through them, and find the one labeled, B<AmazonSESFullAccess> and check the policy. 
...
Textboxes to plug in the B<Endpoint>, B<Access Key ID> and B<Secret Access Key> we just retrieved will be revealed. 
Plug in those two keys, and test them by clicking the button labeled, B<Test Your AWS Credentials>.
...
Log into your mailing list and go to, B<Sending - Options> 

Select, B<Amazon Simple Email Service>
...
For Dada Mail, that means verifying the List Owner, and if you're using the Bounce Handler, perhaps the List Administration email address:  
...
To make things easier, Dada Mail can be set to automatically adjust its Batch Settings to work within your limit, but not go above it. 
...
Keeping the mass sending rates high will show the Amazon AWS system that you're utilizing the system, and they're most likely going to want to raise the limit for you. 

=head3 Verifying a Domain with Amazon SES

Verifying a Domain with Amazon SES allows you to use any email address belonging to your domain that you would like for your List Owner and List Admin. 
This makes setting up the Bounce Handler with Dada Mail a lot easier, as it can be tricky to verify the Bounce Handler email address, if the Bounce Handler plugin itself keeps checking the account, then removing the messages it reads.  
...
To verify a domain, log back into your AWS console, and go to Services >> SES. 
AWS has the idea of Endpoints, so you want to make sure you do all of the following work on the B<same> endpoint that you've set up Dada Mail to utilize. 
In this doc, we're using the, B<US West (Oregon)> endpoint. On the left hand menu, click the, B<Domains> link, then click the, B<Verify a New Domain> button.
...
and AWS will moitor your DNS records to see if those records are set up. 
...
Our website is managed via cPanel - and so is our DNS settings. In our case, we would log into our cPanel, and go to B<DNS Zone Editor>.
Under, B<Add DNS Record>, we would put:

	_amazonses.dadademo.com

Change the B<Type> to, C<TXT>, and put: 
...
Make sure under, B<Type> to select, C<CNAME> and not, C<TXT>. 
...
Once the form is submitted, it'll take approx. one day to get an answer. Once you do get a positive answer, you're ready to use Amazon SES in production. Hurray! 

...
In the C<dada> directory, rename the, "installer-disabled" directory to simply, C<install> and change into this directory. 
...
You're now ready to run the C<install.cgi> install/configuration script and below are instructions 
on how to do that. 

To make sure permissions are set correctly,  we suggest running the C<install.cgi> script under the same owner as 
will run Dada Mail (or change the ownership of the files at a later on - just don't forget!)

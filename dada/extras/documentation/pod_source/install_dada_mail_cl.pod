...
Installing with the CLI requires command line access (just to be perfectly clear)
...
Uncompress Dada Mail - this will create a new directory named C<dada>. Change into 
this directory:

    gunzip dada-8_0_0-alpha3.tar.gz
    tar -xvf dada-8_0_0-alpha3.tar
    cd dada

In the C<dada> directory, rename the, "installer-disabled" directory to simply, C<install> and change into the 
directory. Once in the C<installer> directory,change the permissions of the C<install.cgi> 
script to, C<755>: 
...
Below are instructions on how to use the C<install.cgi> script. To make sure permissions are set correctly, 
we suggest running the C<install.cgi> script under the same owner as will run Dada Mail, 
or change the ownership of the files at a later on - just don't forget!

=head1 Introduction to the Installation/Configuration Process

The C<install.cgi> script takes care of the following things: 
...
keep its temporary files, amongst other things. If you use SQLite for your backend, 
the SQLite database will be saved here, as well. 

=item *  Writes the .dada_config configuration file. 

Dada Mail's global configuration file is called, C.dada_config> and is 
written in Perl. Any global changes you would like to make to Dada Mail should be 
made in this file. The global defaults are located in C<dada/DADA/Config.pm>, it 
is not suggested that you make changes to this file. 

=item * Copies over static files

Dada Mail has a collection of static files that need to be copied over and in some 
cases also configured to work. Static files include images, stylesheets and javascript files - 
some of these need extra configuration. Dada Mail keeps all of these files in a directory 
called C<dada_mail_support_files>. The collection of these files will be copied from the, C<dada/static> 
directory from the distribution, but the design is not to configure Dada Mail to use the location 
as the location for your C<dada_mail_support_files> directory> - they should most 
definitely be located outside of the C<dada> directory. 

=item * Verifies configuration information

The installer will make sure that the information you submit is correct - files 
can be written where you ask them to be written, the database connection works, etc. 

=back

The C<install.cgi> script does not take care of any web server configuration, so for example, if you run Dada Mail under PSGI/Plack, you will need to configure your webserver (nginx, most likely) to run Dada Mail, once you start the PSGI process. 

More on running Dada Mail under nginx:
...
are UPGRADING Dada Mail, there are separate options to make things easier: 
...
Set C<--dada_files_loc> to the parent directory you would like the .dada_files 
directory to be created in. 
 
=head2 --program_url

The URL used to access Dada Mail. 
...
so you will not need to pass this parameter.
...
=item * --wysiwyg_editor_install_ckeditor

Installs CKeditor

=item * --wysiwyg_editor_install_tiny_mce

Installs Tiny MCE
	
=item * --file_browser_install
	
pass either, C<kcfinder> to install the KCFinder File Browser,
...
Dada Mail support for using Amazon SES to send messages out, sending verification 
email messages to addresses you would like to use the service, and to find out 
your sending stats and limitations. To do that, you will need to pass your 
C<AWSAccessKeyId> and C<AWSSecretKey>.

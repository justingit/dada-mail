...
Pro Dada Subscriber? Download Pro Dada at: 
...
If you're doing an upgrade, you may simply use the same location of your C<.dada_files>. Instead of filling in any of the configuration below, you may simply click the, B<Configure Dada Mail!> button on the bottom of the installation screen. 

When you do so, the installer will refresh the screen with the same configuration screen, but two new options, under another warning that the installer couldn't create the, C<.dada_files> directory. 
...
This second option may be more useful in helping you keep a current C<.dada_config> file. In many cases, upgrades to Dada Mail come with changes to the default C<.dada_config> file. These changes may include incompatibilities if you use your current C<.dada_config> file. 
...
Some of the plugins/extensions have additional configuration that will need to done, before they work completely. For the majority of these plugins/extensions, you'll need to also set a cron job. One plugin, the bounce handler, also requires you to fill out the credentials for the bounce handler email account you need to set up to capture bounced emails. 

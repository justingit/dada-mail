...
Here's the direct download to Dada Mail v5.2.0 

L<https://github.com/downloads/justingit/dada-mail/dada-5_2_0.tar.gz>
...
=head1 Step #2 Upload the, "dada-5_2_0.tar.gz" distro to your cgi-bin

Download the C<dada-5_2_0.tar.gz> distro (you will B<not> have to uncompress the distribution) and then upload it to your hosting account's I<cgi-bin>.
...
<ul>
<li><strong><a href="https://raw.github.com/justingit/dada-mail/v5_2_0-stable_2012_07_26/uncompress_dada.cgi">uncompress_dada.cgi</a> (Save Page/Link As...)</strong></li></ul>

=end html

and upload this script (PlainTest/ASCII mode - NOT binary!) to your hosting account's I<cgi-bin> - right alongside the I<dada-5_2_0.tar.gz> distro you just put up. 
...
The, C<uncompress_dada.cgi> should attempt to uncompress the I<dada-5_2_0.tar.gz> distro, change the necessary permissions of various files and prep the actual installer for you to use. 
...
<pre>
Can't find dada-5_2_0.tar to uncompress!
...
Optionally, you may also configure the WYSIWYG editors that come bundled with Dada Mail. These WYSIWYG editors will allow you to author and edit your HTML Mailing List Messages right in your List Control Panel. 

=head3 Support Files Directory

The only thing that the Installer needs to install and configure the WYSIWYG editors, is a location to install them at. The installer will need this location in two forms, the B<Absolute Directory on where to place the support file directory> and the B<Corresponding URL> to that directory. 

Unlike the C<.dada_files> directory, you want to be able to access this directory via your web browser, since the WYSIWYG editors are little web-based apps in their own right. The File Browser (KCFinder) allows you to upload images to use in your mailing list messages, and those images will also need to be publically accessable. 

By default a I<pretty good guess> is made, to set both the server absolute path to your public HTML directory, as well as the corresponding URL to access this directory via a web browser. If the default location looks fine, keep it as-is. If you'd like to customize/fix where this directory lives, please do so. Under this directory, the Installer will make new directory called, C<dada_mail_support_files> and under this directory, install the WYSIWYG editors (C<ckeditor>, C<tiny_mce>, C<fckeditor>, the File Browser/Image Uploader (C<kcfinder>), as well as the directory that images/files are uploaded to, (C<file_uploads>. 

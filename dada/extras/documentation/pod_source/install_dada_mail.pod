...
<p>
    <li>
    <a href="https://sourceforge.net/projects/dadamail/files/dada-9_0_0-alpha1.tar.gz/download">
        Click here to download to Dada Mail
    </a> 
    </li>
    <li>
    Or, if you're a Pro Dada Subscriber, 
    <a href="http://dadamailproject.com/pro_dada">
        click here to download to Pro Dada
    </a>
    </li>
</ul>
</p>
    
    <p><a href="http://dadamailproject.com/purchase/pro.html">
    Here's more information about Pro Dada
    </a>
    </p>
...
I<When installing Pro Dada, use, B<pro_dada-9_0_0-alpha1.tar.gz> as the name of the distribution, in these directions.>


=head2 Step #2 Upload the, "dada-9_0_0-alpha1.tar.gz" distro into your cgi-bin

Once you downloaded the C<dada-9_0_0-alpha1.tar.gz> distro, upload it to your hosting account's I<cgi-bin>.
...
<ul>
<li><strong><a href="https://raw.github.com/justingit/dada-mail/v9_0_0-alpha1-stable_2015_10_27/uncompress_dada.cgi">uncompress_dada.cgi</a> (Save Page/Link As...)</strong></li></ul>

=end html

and upload this script (PlainTest/ASCII mode - NOT binary!) to your hosting account's I<cgi-bin> directory - right alongside the I<dada-9_0_0-alpha1.tar.gz> distro you just put up. 
...
The, C<uncompress_dada.cgi> should attempt to uncompress the I<dada-9_0_0-alpha1.tar.gz> distro, change the necessary permissions of various files and prep the actual installer for you to use. 
...
<pre>
Can't find dada-9_0_0-alpha1.tar to uncompress!
...
Dada Mail will need to save its mailing lists and other information inan SQL database.

=head3 Choices: MySQL, PostgreSQL or SQLite

Both B<MySQL> and B<PostgreSQL> will have the same features available and work under the hood, basically the same way. We recommend either equally.
...
This database has to be set up beforehand. It's not too hard to do if you are running an account under cPanel w/phpMyAdmin to help you along. Just note the B<Server>, B<Database>, B<Username> and B<Password>, once you create the database (You will not need to create any tables beforehand). The SQL user will need full priviliges to your database. For security reasons, we don't suggest giving this same user any permissions to any other database. 
...
One notable exception is B<Amazon SES support>. 

=head2 Completing the Installation and Configuration. 

Once you've filled out the form completely, just click the, B<Configure Dada Mail!> button. The screen will refresh and go about configuring Dada Mail for you.

If everything works well (and we hope it does) you should be all set to run Dada Mail.

=head2 Setup the Scheduled Job (cronjobs)

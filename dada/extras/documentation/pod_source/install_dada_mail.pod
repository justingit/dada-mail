...
Here's the direct download to Dada Mail v4.6.0 alpha 2: 

L<https://github.com/downloads/justingit/dada-mail/dada-4_6_0-alpha2.tar.gz>
...
=head1 Step #2 Upload the, "dada-4_6_0-alpha2.tar.gz" distro to your cgi-bin

Download the C<dada-4_6_0-alpha2.tar.gz> distro (you will B<not> have to uncompress the distribution) and then upload it to your hosting account's I<cgi-bin>.

I<In these directions we suggest the cgi-bin, but feel free to use any directory that you B<know> will work.>

=head1 Step #3 Upload the, "uncompress_dada.cgi" script to your cgi-bin

Download the, C<uncompress_dada.cgi> script from here: 

=for html <ul><li><strong><a href="https://github.com/justingit/dada-mail/raw/v4_6_0_alpha2-2011_07_28/uncompress_dada.cgi">uncompress_dada.cgi</a> (Save Page/Link As...)</strong></li></ul>

and upload this script (PlainTest/ASCII mode - NOT binary!) to your hosting account's I<cgi-bin> - right alongside the I<dada-4_6_0-alpha2.tar.gz> distro you just put up. 

=head1 Step #4 Change permissions of the, C<uncompress_dada.cgi> file to, B<755>

=head1 Step #5 Visit the, C<uncompress_dada.cgi> script in your web browser. 

The, C<uncompress_dada.cgi> should attempt to uncompress the I<dada-4_6_0-alpha2.tar.gz> distro, change the necessary permissions of various files and prep the actual installer for you to use. 
...
If you are upgrading Dada Mail, you may type in the current location of your, C<.dada_files> directory. The installer may also see that it's "best guess" is
...
Fill out the form fields of the installer, like you were doing a installation. Click the, "Configure Dada Mail!" button. 
...
=head4 Use the existing directory structure and configuration file, ignoring the configuration below.

The first option, I<Use the existing directory structure and configuration file, ignoring the configuration below.> will leave your Dada Mail configuration alone, and simply allow you to upgrade to the latest version of Dada Mail.

If you have any plugins/extensions that you've installed with the installer from a previous installation, the installer will try its best to at least change the permissions of each plugin/extension you have already installed to continue working. It's possible that the installer I<cannot> detect a plugin/extension install, so you may have to reinstall the plugin/extension manually. 

=head4 Use the existing directory structure, but create a new configuration file, using the configuration below

The second option, I<Use the existing directory structure, but create a new configuration file, using the configuration below> allows you to upgrade your current Dada Mail, while using a fresh copy of the C<.dada_config> file. 

This second option may be more useful in helping you keep a current C<.dada_config> file. In many cases, upgrades to Dada Mail between minor dot releases (for example, v4.B<5>.0 and v4.B<6>.0 come with changes to the default C<.dada_config> file. These changes may include incompatibilities if you use your current C<.dada_config> file. 
...
Select B<Default> if you cannot, don't know how, or do not want the SQL backend option I<(we really suggest going the distance and using the SQL backend)>

=head4 MySQL or PostgreSQL

Both B<MySQL> and B<PostgreSQL> will have the same features available and work under the hood, basically the same way. We like PostgreSQL better, but MySQL is going to be tested a bit more thouroughly and probably is more familiar to most people, so at the moment, it's the backend that we recommend. 
...
If you have an already existing database from a previous Dada Mail installation, you may fill in the SQL server login information for this existing database and Dada Mail will use it, instead of creating a set of new tables for you. This is particularily useful if you're B<moving> Dada Mail from one server to another. 
...
One plugin, the bounce handler, also requires you to fill out the credentials for the bounce handler email account you need to set up to capture bounced emails. 

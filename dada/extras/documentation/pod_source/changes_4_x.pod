...
This welcome message can be customized in the list control panel in the, B<Appearance and Templates - Email Message Templates> screen under, B<Subscribed by List Owner Message> I<(Should be simply, Welcome from List Owner message?)>
...
Mass unsubscriptions in the list control panel I<(if this option is enabled)> can automatically be followed up with a, "Farewell" message sent to only addresses that have just been removed. 

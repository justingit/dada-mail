...
Under the, B<Enable Digests> setting, you may set the frequency of the messages sent out. Digest messges will not be sent out, if there are no messages sent between the last digest sent out, and whenever Dada Mail checks for new digests to be sent out. 
...
In the List Control Panel navigate to, Membership - View and click on the address you would like to work with. Click the, B<Delivery Preferences> tab and choose among the three options for delivery: B<Receive All Messages>, B< Receive Messages in a Digest>, or, B<Hold Mailings>.
...
The following tag are supported: 

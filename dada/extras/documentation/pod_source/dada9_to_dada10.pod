...
These modules and other, optional CPAN Perl modules used by Dada Mail have been bundled together in one CPAN Module called, C<Bundle::DadaMail>. You can now just install this one module, and have all those other modules installed in one step. 
...
we've removed the ability to edit these messages from within Dada Mail through the B<Apperance - Email Message Templates> screen. 
The HTML produced by the  framework is exceptionally complex - they just cannot be easily edited by hand. 

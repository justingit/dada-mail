...
B<Don't put off upgrading if you are required to comply with the GDPR!> 
...
It's a B<great> read, and if you want to get into the nuances of consent under, GDPR, it's a I<must-read>. 
There's many opinions on the Internet on what exactly constitutes GDPR compliance - we even have our own. 
Our advice though, is to only take the advice of a knowledgable lawyer if you have questions.   
...
In Dada Mail, 
these points of consent are I<never> prechecked, and are I<always> required. 

=head2 Adding/Removing Consent
...
you'll have to abide by them I<yourself>
  by only use the information you've collected in a way that your subscribers have given their consent to. 
...
they've agreed in granted to you every point of consent you've asked from them. 

If a user I<hasn't$cl_consent>, 
...
(Dada Mail will always place an unsubscribe link into your messages). 
...
=head1 Changes to the Application
...
Our suggestion is that every public mailing list should comply with the GDPR, 
without exception. 

=head2 Migrating Existing Subscription Information 

Older versions of Dada Mail I<did> keep a plaintext log of subscription and unsubscription activities of the mailing lists it managed, 
including when subscription requests were made for closed-loop opt-in confirmed subscriptions. 

We've created a tool to import some of that information into Dada Mail's new, 
explicit consent activity database, 
to help flesh out history of consent you have had with your subscribers.

B<This import tool can't retroactivity allow you to somehow have your subscribers give you I<consent> that they haven't given before> 
and using this tool alone won't make your mailing list magically GDPR compliant. 
It only moves the history of who was added to your mailing list, 
and when. 

The import tool also isn't perfect. 
If your current mailing list had some sort of history where "confirmed-by-the-user" 
(via Dada Mail's closed-loop opt-in system) subscriptions was not required, 
think long and hard about using this tool on your mailing list at all.  
...
B<usage_log_to_consent_activity> and check the option next to its name to have it installed along with the rest of Dada Mail. 
...
the plugin also has a command line interface: 

Connect to the server you have Dada Mail installed, 
and change into the "plugins" directory, where you'll find this plugin (usage_log_to_consent_activity)
...
to set up the Privacy Policy for each list and make sure it does not get changed, if they so desire
...
alone, 
in its own screen. 
Again, this is to make it easier to enable/disable when logged in with the List Password. 
...
as the history of what a consent was a particular date and time is important. 

They may be deleted, 
but it's highly advised to NOT do this (unless you're setting things up, and moving things around),
...
it could be a useful tool to utilizse to re-confirm subscribers from an old mailing list to a new mailing list
that has consent and privacy policy set up correctly. 
...
This is not allowed under GDPR (no explicit consent), 
so unfortunetely we replaced this button with a button that takes you to a subscription form 
...
"Magic Subscription Forms" would fill in a logged in user (using their Dada Mail Profile) with their email address. We've removed this feature to 
simplify things, and to closely follow the "no pre-filled in" points of consent of the GDPR. 
...
Here are the steps you'll need to reconfirm your mailing list using Dada Mail. 
...
Log into your list control panel into the mailing list you want to work with. Go to: B<Membership: View> 

At the bottom of the screen click the button labeled, B<Export Subscribers (.csv)>. The only option you will need checked is, B<Email Address>.
...
For completeness, make sure to use a different B<list short name> than before, 
...
and the B<Consents> that you would like your users to agree in giving you. 
Only one consent can be set up during list creation, 
so if you need more, 
do so afterwards. 

=head2 Confirm your Privacy Policy and List Consent setup

Get this right part right! 
...
and shows all the addresses that past the verification process, 

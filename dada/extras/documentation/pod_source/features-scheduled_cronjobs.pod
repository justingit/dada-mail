...
Only one cronjob currently needs to be run for all the features in Dada Mail needs a scheduled job to run.just a command to grab a URL (curl, wget, etc) and the URL to access.

Here's an exmaple: 

L<http://example.com/cgi-bin/dada/mail.cgi/_sched513A3F128C5C41EC/_all/_all/_verbose/>

C<http://example.com/cgi-bin/dada/mail.cgi> is just the URL to your Dada Mail, 

A full cronjob would look like this: 

    */5 * * * * /usr/bin/curl  -s --get --url http://example.com/cgi-bin/dada/mail.cgi/_sched513A3F128C5C41EC/_all/_all/_silent/

C<*/5 * * * * > sets the schedule to run this job every 5 minutes, 

C</usr/bin/curl  -s --get --url> is the curl command, with different paramaters passed to grab the URL and, 

C<http://example.com/cgi-bin/dada/mail.cgi/_sched513A3F128C5C41EC/_all/_all/_silent/> 

is the URL we want to grab. We've modified it slightly from what was originally shown (C<_silent>, instead of, C<_verbose>), which we'll explain below. 
...
If you would like to customize, the value of this paramater should only contain letters, numbers and underscores. Anything too tricky will foil the run mode dispatch code of Dada Mail. 
...
There's no special command-line way to run these jobs, although you can invoke Dada Mail on the command line easy enough. The following: 
    ...
    cd /home/youraccount/public_html/cgi-bin/dada; /usr/bin/perl ./mail.cgi flavor=_sched schedule=_all list=_all output_mode=_verbose

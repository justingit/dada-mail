...
Cronjobs created prior to v8 are no longer used in Dada Mail, so if you are upgrading, you will need to reset your cronjobs: remove the one's that exist for Dada Mail, and add the one new cronjob that's now needed. 
...
=head3 Bridge: using Email Forward for List Emails - Broken

Mail Forwarding for List Email current does not work in v8. The command line interface has been removed from Bridge, as it has for all plugins. This leaves the problem of how to make a mail forward to pipes to a command. 

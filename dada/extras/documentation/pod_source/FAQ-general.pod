...
We highly suggest using the installer that comes with Dada Mail, but you may install 
Dada Mail manually, as well. 
...
I<Is there a way to Mass-Subscribe email addresses in the list control panel?>

The best way to mass-subscribe email addresses and the only way we suggest, is to B<Send a List Invitation>. This makes sure you stick with a closed-loop opt in system, which itself makes sure only people who want to subscribe to your mailing list, will be. To run a successful and sustainable mailing list, this a closed-loop opt in system is tantamount. 

Several different scenarios are explained, using the closed-loop, opt in system: 

L<http://dadamailproject.com/installation/using_dada_mail.html>


We do understand that this causes a headache when moving from a previous mailing list system, to Dada Mail, since the addresses may already be verified with the previous system, using a closed-loop opt in system and sending a List Invitation to all these addresses essentially requires each individual subscriber to confirm twice. 

We'd really like your opinions on how to make this type of import of a previous mailing list available, without having Dada Mail also open for abuse by people who would rather just mass-subscribe unconfirmed email addresses, which we do not condone. Please join the current discussion about this, here: 

L<http://dadamailproject.com/support/boards/viewtopic.php?f=25&t=2457>

Enabling mass-subscribing via the list control panel is still an option. 

For each mailing list where you want mass subscribing in the list control panel available, log into your mailing list with the B<Dada Mail Root Password> and go to, 

I<Your List Control Panel - Options> 

Check the option labeled, B<Enable, "Subscribe Checked Subscribers" button [...]>

Although I<once> slated for deprecation, we're instead looking at alternatives to keep Dada Mail flexible, yet still not so appealing to be used for abuse. Again, we could really use your feedback.
...
This will provide a link to unsubscribe, but the user will have to manually enter their email address. 

=head2 Unubscription links are placed on top of my mailing list messages, why? 
...
There are chances that a mailing will stop unexpectantly, and keeping this monitor up will prevent that. There are also ways to not have to have this Monitor Your Mailings screen open, regardless of your hosting setup. 

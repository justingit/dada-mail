...
=head3 Install Dada Mail via Simple Scripts
...
=head2 Can I install more than one copy of Dada Mail on the same hosting account? 

Yes. 

The Dada Mail Installer usually picks your home directory as the location for the C<.dada_files> directory. If you choose to install another copy of Dada Mail, the installer will then want to use this same (and currently in-use) location. 

The trick is to simply tell the installer to use a different location. For example, create a new directory in your home directory (call it something like C<dada_files_for_xyz> and then point the installer to that directory (use the I<Manual> location, instead of I<Automatic>) and you're done!. 

=head2 Can I use one Dada Mail installation for multiple domains? 
...
Log into your mailing list using the B<Dada Mail Root Password>.

In the left-hand menu, go to B<Your Mailing List -E<gt> Delete This Mailing List >
...
=head2 How do I change the list short name? 
...
Before running, make a full backup of your Dada Mail SQL database, as changes could potentialy be irreversable.
	...
	UPDATE dada_archives SET list = 'new' WHERE list = 'old'; 
	
	UPDATE dada_bounce_scores SET list = 'new' WHERE list = 'old'; 
	
	UPDATE dada_clickthrough_url_log SET list = 'new' WHERE list = 'old'; 
	
	UPDATE dada_mass_mailing_event_log SET list = 'new' WHERE list = 'old'; 
	
	UPDATE dada_settings SET list = 'new' WHERE list = 'old'; 
	
	UPDATE dada_settings SET value = 'new' WHERE value = 'old' and setting = 'list';
	
	UPDATE dada_subscribers SET list = 'new' WHERE list = 'old'; 

In the above SQL, change, C<new> to the new list short name you would like to use and, C<old> to the old list short name you would like to rename.
...
=item * is all lowercase

=item * is no more than 16 characters 

=item * only consists of alpha-numeric characters
...
=item * Update any subscription forms you may have embedded on your site's pages

=item * Update any plaintext logs that Dada Mail keeps

=item * Update any subscription or unsubscription links

These links in already existing emails will be B<broken>.
...
the Dada Mail Root Password is stored your C<.dada_config> file. 

I<(The location of the C<.dada_config>> file is saved in the directory set in the C<dada/DADA/Config.pm> file, in a variable called, C<$PROGRAM_CONFIG_FILE_DIR>. That variable should hold the absolute path to the directory that your, I<.dada_config>)
...
In this example, the C<$PROGRAM_ROOT_PASSWORD> variable holds an encrypted password as C<$ROOT_PASS_IS_ENCRYPTED> is set to, C<1>. There's no way to decrypt the password.

You can encrypt a new password by visiting your Dada Mail with the query string, C<?f=pass_gen>, like this: 

L<http://example.com/cgi-bin/dada/mail.cgi?f=pass_gen>

Where, I<http://example.com/cgi-bin/dada/mail.cgi> is the URL of your Dada Mail and have a new password encrypted. Copy the encrypted password and set the C<$PROGRAM_ROOT_PASSWORD> variable to your new encrypted password. 

If you cannot figure out how to encrypt your own password, set, C<$ROOT_PASS_IS_ENCRYPTED> to, C<0> and type in a new Dada Mail Root Password: 
...
If you are running a Basic Installation of Dada Mail (ie: NO .dada_config file), look for the C<$PROGRAM_ROOT_PASSWORD> variable in the C<dada/DADA/Config.pm> file itself. It may or may not be encrypted. Again, the 
C<$ROOT_PASS_IS_ENCRYPTED> will let you know. follow similar steps to reset the C<$PROGRAM_ROOT_PASSWORD> variable to either an encrypted or non-encrypted password, but do all the editing in your C<dada/DADA/Config.pm> file. 
...
=head2 How are new subscriptions made? 

=over

=item * List Invitations

The first way is by having the list owner log into the List Control Panel and inviting subscribers en-mass, via the, B<Manage Subscribers - Invite> screen. 

=item * Self-Subscriptions
...
L<http://www.youtube.com/watch?v=kUENzRN2EPA&hd=1>

=back

=head3 List Invitations

Once logged in, the list owner will want to visit the, B<Manage Subscribers - Invite> screen. 
...
You can also place this subscription form on any page of an already existing website. The HTML form code can be copied from the List Control Panel screen, located at: B<Appearance and Templates - Subscription Form HTML>
...
=head2 Mass Subscribing Email Addresses via the List Control Panel

I<Is there a way to Mass-Subscribe email addresses from the list control panel?>

B<Versions 4.7.0+>:

Log into your mailing list and go to: B<Your Mailing List - Mailing List Options>

Under the heading, B<List Control Panel Subscriptions> check the option, B<Enable Adding Subscribers from within the List Control Panel> 

You can then go to: B<Your Subscribers - Invite/Add> and mass-subscribe 
addresses. 

B<Versions 4.4.0 - 4.6.1>

Log into your mailing list with the B<Dada Mail Root Password> and go to: 

B<Your List Control Panel - Options>

Check the option labeled, B<Enable, "Subscribe Checked Subscribers" button [...]>
...
=head2 How do I turn off, "one-click" unsubscription links? 

I<I have a discussion list and the unsub links are being clicked on by other users, sending unsubscription confirmations out. Yikes!> 
...
=head2 Unsubscription links are placed on top of my mailing list messages, why? 

Dada Mail has an in-program policy of requiring the unsubscription link to be prsent 
in any mass mailing messages it sends out. This is to stop blatant abuse of the program, but we also 
do sincerely suggest this as a best practice when using the program. If an unsubscribe link is not found 
in your mailing list message, Dada Mail will simply add one to the top of the message. 

The following unsubscription links/tags can be used: 

=over

=item * Path Info style, embeds in the email address of the subscriber
...
=item * Path Info style, DOES NOT embed the email address of the subscriber

	<!-- tmpl_var PROGRAM_URL -->/u/<!-- tmpl_var list_settings.list -->

=item * Query-String style, does NOT embed the email address of the subscriber: 
...
Dada Mail can also be set up to allow discussion lists, you'll have to also install the B<Dada Bridge> plugin. 
...
See the B<Bounce Handler> plugin: 
...
Log into your list control panel, click, B<Appearance and Templates -  Subscription Form HTML>
...
L<http://demo.dadamailproject.com>
...
=item * The, "Send a Message" screen

In the list control panel, go to, B<Send a Message>. Twiddle the B<HTML Version> into view by clicking, 
...
In the list control panel, go to, <Send a Webpage>. Under, B<HTML Version (Your Webpage)>, check: I<Grab content from this webpage address (URL):> and fill in the url to use. 
...
I<I want to upload images to then put in my message - any help on that?>
...
=head2 How do you use TinyMCE with Dada Mail? 

I<(The below instructions work for version 4.9.x of Dada Mail)>

There's no built-in support for TinyMCE, but you may edit Dada Mail's template files to add in support yourself. 

B<Install> TinyMCE. Here's a simple walkthrough:

L<http://www.tinymce.com/wiki.php/%22For_Dummies%22>

B<Edit> the, C<dada/DADA/Template/templates/admin_template.tmpl> file. Add the following lines in the head of the HTML: 

	<script language="javascript" type="text/javascript" src="http://example.com/tinymce/jscripts/tiny_mce/tiny_mce.js"></script>
...
Make sure to change the URL, C<http://example.com/tinymce/jscripts/tiny_mce/tiny_mce.js> 
...
B<Edit> the, C<dada/DADA/Template/templates/html_message_form_field_widget.tmpl> file. Change the contents of the file to the below:

	<!-- begin html_message_form_field_widget.tmpl --> 
	
	<textarea name="html_message_body" cols="80" rows="30" id="html_message_body" class="mceEditor"><!-- tmpl_var html_message_body_content --></textarea>
...
=head2 How do you install, "CPAN Perl Modules"

There are many advanced features in Dada Mail, that rely on I<CPAN Perl Modules>

CPAN is the I<Comprehensive Perl Archive Network> - it's a great collection of reusable Perl code.
...
L<http://perl-begin.org/topics/cpan/life-with-cpan>
...
=head2 How do you change the Dada Mail Logo? 
...
Place it in the, I<dada/DADA/Template/templates/img> directory. 

I<Or> place it in your I<.dada_files/.templates/img> directory. This copy will override the copy that comes with Dada Mail and any additional upgrades of the script will 
not overwrite the custom version of the file you want to use. (This works for all templates located in the I<dada/DADA/Template/templates> directory)

Remove your screen cache (either delete the, I<.dada_files/.tmp/_screen_cache> directory, or use the Screen Cache plugin) to see your changes take effect. 

=head2 Can I change the Program's Name? 

Add this line to your C<.dada_config> file: 

	$PROGRAM_NAME = 'My Program'; 
	
Where, B<My Program> is the new name you'd like to use. 
...
I<just a normal HTML file with a few extra tags, the simplest being C< E<lt>  TMPL_VAR E<gt> >>
...
This is the preferred method. Simply make a copy of the list_template.tmpl file, save it in a another location on your server and set the Config.pm variable, B<$USER_TEMPLATE> to the absolute path of its location. 
...
=item * Make a copy of list_template.tmpl and place it in your $TEMPLATES directory
...
Either, don't edit the individual copy of the list template in the admin control panel under, B<Appearance and Templates - Your Mailing List Template>, check the option, " Use the default list template."  in that same screen, or, to be even more authorative, add this to your %LIST_SETUP_OVERRIDES Config.pm hash: 
...
The information about the Default List Template also follows for the Default Admin Template. The Config.pm variable for it is: B<$admin_template>
...
HTML screens are control by the Config.pm variable, C<$HTML_CHARSET>. You can change this to whatever you'd like. For example, if I'd want to change this to, B<UTF-8> I'd write: 
...
=head2 What are the benefits of using the SQL backends?
...
To change the mailing list message template, login into your list control panel and navigate to: B<Appearance and Templates - Email Message Templates>
...
These two messages can be changed globaly in the Config.pm file, under the variables: B<$MAILING_LIST_MESSAGE> and B<$MAILING_LIST_MESSAGE_HTML> 

If a list already has edited any of the email message templates, your global changes will not take affect for that list. 

=head2 Are the messages I send with Dada Mail traceable back to me? 
...
Yes, we do not suggest it. Dada Mail's, C<Invite/Add> admin screen does a good job in keeping your list's clean of invalid email addresses, duplicates, blacklisted addresses - etc. 
...
=head2 Tuning Dada Mail to Send Mass Mailings Faster 

=over

=item * Install HTML::Template::Pro

L<./features-html_template-pro.pod.html>

C<HTML::Template::Pro> can render email templates faster than the included C<HTML::Template>

=item * Do not use the advanced email templating syntax

The advanced email templating syntax engine (C<HTML::Template::Expr>) is much slower than the regular syntax. 

Uncheck, B<Enable use of the advanced templating syntax for email messages.> in I<Appearance and Templates - Email Message Templates> (under, I<Advanced Options
>). Make sure there are no email messages that use any, CE<lt>!-- tmpl_expr ... --E<gt>> tags. 

=item * Set the config variable, $MIME_OPTIMIZE to, 'faster'

Add this to your C<.dada_config> file: 

	$MIME_OPTIMIZE = 'faster'; 

=item * Offload your mail sending to a separate mail server or mail service

This could mean different things for different setups. You may want to use a separate SMTP server, either your own, or a 
SMTP host, like SendGrid (L<http://sendgrid.com>). If you do use SMTP to send message, enable this option: 

B<Use One SMTP Connection per Batch (Experimental)>

In, I<Mail Sending - Mass Mailing Preferences>

You could also experiment with using B<Amazon SES> for mail sending. See:

L<./features-amazon_ses_support.pod.html>

Using a separate mail service, like Sendgrid or Amazon SES will allow you to go over any hourly/daily email limitation imposted by your hosting company, 
since you I<are not> sending messages through this host's mail servers.

=item * Use the Bounce Handler plugin to keep a clean mailing list

Invalid email addresses on your list just make mass mailing go slower. Use the bounce handler 
to remove those addresses from your mailing list: 

L<./dada_bounce_handler.pl.html>

=item * Adjust your batch settings


Adjust your batch settings to send more messages out per batch and/or wait less time between batches. 
It's not recommended that you totally I<disable> batch sending, but if you do not have a hourly/daily mail sending 
limit, you can certainly bump up the batch settings to accomodate the resources you have available. 

=back

=head2 Screen Load Times

Dada Mail loads its images, javascript and css files through the app itself. Requests for these types of 
resources fires up a copy of Dada Mail, which then serves it up.

Dada Mail's screen load times can be significantly faster, if you instead copy these types of resources to a different 
location and call them directly. 

In these examples, we'll use: B<http://example.com/static_dada_files> as the location we're going to copy these resources to.

Before beginning this, be warned that these files may be changed between versions, so these steps would have to be taken every time 
you upgrade Dada Mail. 

=head3 css files

Dada Mail's css files are in the, I<dada/DADA/Template/templates/css> directory. Copy this directory and place this copy in 
a different location of your hosting account, where the files can be served statically.

In these examples, we'll use: B<http://example.com/static_dada_files/css>

In the B<list_template.tmpl> file, find: 
...
In the B<admin_template.tmpl> file, find: 
...
In our example, we would change this to: 
  ...
  <link rel="stylesheet" href="http://example.com/static_dada_files/css/default.css" type="text/css" media="screen" />
<!-- tmpl_if load_modalbox --> 
  <link rel="stylesheet" href="http://example.com/static_dada_files/css/modalbox.css" type="text/css"  media="screen" />
<!-- /tmpl_if -->
...
Dada Mail's image files are in the, I<dada/DADA/Template/templates/img> directory. Copy this directory and place this copy in 
a different location of your hosting account, where the files can be served statically.

In these examples, we'll use: B<http://example.com/static_dada_files/img>
...
There are other images you may swap out for their static kin - do a directory search in, I<dada/DADA/Template/templates> for the following: 

	<!-- tmpl_var PROGRAM_URL -->/img/

And replace this with your static URL, in a similar way. 

=head3 javascript files

Dada Mail's javascript files are in the, I<dada/DADA/Template/templates/js> directory. Copy this directory and place this copy in 
a different location of your hosting account, where the files can be served statically. 

In these examples, we'll use: B<http://example.com/static_dada_files/js>
...
In our example, we would change this to: 
	  ...
	  <script src="http://example.com/static_dada_files/js/dada_mail_admin_js.js"         type="text/javascript"></script>
	  <script src="http://example.com/static_dada_files/js/prototype_scriptaculous_package.js" type="text/javascript"></script>
	
	<!-- tmpl_if load_modalbox --> 
	  <script src="http://example.com/static_dada_files/js/modalbox.js"                   type="text/javascript"></script>
	<!-- /tmpl_if --> 
...
=head3 Only Archive, Don't Send, method
...
=item 5 Check: B<Only Archive, Don't Send>

...
The best way to mass-subscribe email addresses is to send a List Invitation. This makes sure you stick with a closed-loop opt in system, which itself makes sure only people who want to subscribe to your mailing list, are. To run a successful and sustainable mailing list, this is tantamount. 

Several different scenarios are explained, using this system, here: 

L<http://dadamailproject.com/installation/using_dada_mail.html>

Past version of Dada Mail did allow you to subscribe email addresses en-mass in the list control panel, but this feature has been B<deprecated>, for the reasons that closed-list opt in is not I<just> a good idea, it's really the only policy you should follow. 

Enabling the feature is still an option. You'll need to change the Config variable C<$ENFORCE_CLOSED_LOOP_OPT_IN> to something other than, C<1>. You will also have to log into your mailing list with the Dada Mail Root Password and go to, I<Your List Control Panel - Options> and check the option labeled, B<Enable, "Subscribe Checked Subscribers" button [...]>

This deprecated feature will be B<removed completely> in future versions of Dada Mail. 

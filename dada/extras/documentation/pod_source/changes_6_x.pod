...
is a mailing list used in a closed, internal setting; not one used publically 
for marketing purposes. 

B<Example:> a group of colleagues that want to discuss a matter, and do not want this discussion to be public. 

An example of a B<public mailing list> would be a mailing list that anyone is free to subscribe to (and unsubscribe from), used for marketing purposes. 

When a mailing list is set to, B<Private>, unsubscription links are not required (as they are in a public mailing list), but any unsubscription links will be turned into a link to B<Request an Unsubscription>. This request will go to the List Owner, who will have the final say on who is removed from the mailing list. 
...
=head3 Subscriber Timestamp

Subscriber information now includes the B<Timestamp> of when a subscription was made.
...
The B<View List> screen in Dada Mail's list control panel already has a simple search, to find what you're looking for in your Subscribers, but now also includes an, B<Advanced Search>, allowing you to search more specifically for exactly what you're looking for. You may now search by email address, any profile field independently, as well as between subscription date ranges. 

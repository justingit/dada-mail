...
Previous versions of Dada Mail were specifically designed to run B<Public> Mailing Lists: mailing lists where usually anyone can subscribe to using a subscription form, and most I<definitely> everyone can remove themselves from (unsubscribe). 

For public mailing lists and marketing purposes, this works just great, but many users want to utilize the app for a different purpose: utilizing Dada Mail's mailing list organizational and mass mailing features for a I<private> list of contacts: a group of colleagues, or an internal company mailing list.

In this role that Dada Mail wasn't designed for, it hasn't fared very well, as List Owners become frustrated, having to constantly deal with list member removing themselves (often by accident). The tools to run a private mailing list just weren't there, also solutions to this problem feel very hacky.  
...
When a mailing list is set to, B<Private>, unsubscription links are not required (as they are in a public mailing list), but any unsubscription links found in a mass mailing message will be turned into a link to B<Request Removal>. This request will be sent to the List Owner, who will have the final say on who is removed from the mailing list. 

All other options for a mailing list are still available for a Private List.

=head4 To Do

Options on how a mailing list is operated are already complex, and this feature just makes things I<more> complex. Options need to be organized better, and more self-explanitory, or users will get confused and ultimately use the features incorreclty. 
...
=head3 Subscriber Subscription Date saved in a Timestamp
...
This release of Dada Mail also includes an B<Advanced Search>, allowing you to search more specifically for exactly what you're looking for. You may now search by email address, any profile field independently, as well as between subscription date ranges. 
...
=head3 Mass Update of Subscriber Profile Fields, based on Advanced Search
...
=head3 PlainText Backend Removed (Probably)
...
=head3 Dada Mail distro now hosted on Sourceforge
...
=head3 json and jsonp clarifications
...
L<http://dadamailproject.com/d/COOKBOOK-subscriptions.pod.html#jQuery-Plugin>

=head3 Amazon SES New Endpoint/Region Support
...
=head3 List-Unsubscribe Header

We've changed the C<List-Unsubscribe> header to include both a mailto: link set to the List Owner's email address, as well as the full List-Unsubscribe URL. 

Past versions only included the List-Unsubscribe URL. 

=head3 Log Viewer: Timestamps/Labels Fixed for Mass Mailing Logs
...
=head3 .htaccess in file uploads directory
...
Which potentially won't do I<anything>. 
...
=head3 Confirmation Dialog box when only archiving a message
...
=head2 Bugfixes 6.8.1

=head3 Archive Index Previous/Next links not shown

L<https://github.com/justingit/dada-mail/issues/419>

=head3 Authorised senders get an error from bridge.cgi

L<https://github.com/justingit/dada-mail/issues/420>

=head1 6.8.0

=head2 Features 6.8.0

=head3 Rich Text Editors and Web-Based File Browsers
...
=head4 CKEditor Upgraded to v4.3, TinyMCE Upgraded to v4.0.11, FCKeditor retired
...
=head4 CKEditor: Template Tag Dropdown Menu
...
=head4 Core5 Labs FileManager Support
...
More information on B<FileManager:>

L<http://labs.corefive.com/projects/filemanager/>
...
Read about the Bridge plugin: 
L<http://dadamailproject.com/d/bridge.cgi.html>

This version of Dada Mail supports many difference enhancements: 

=head4 Moderators and Authorized Senders
...
=head3 Amazon SES Auto-Adjust Batch Sending Rates
...
Read more about Dada Mail and Amazon SES, see: 
...
=head3 Mass Mailing Logs: save-able and viewable in the Log Viewer plugin
...
=head3 Highlighting in the list control panel's menu
...
=head3 Clickthrough Tracking: URLs in image maps tracked

URLs for links in image maps weren't tracked by Dada Mail. These URLs should now be properly tracked. 

=head2 Bugfixes 6.8.0

=head3 Profile Fields, "Name" validation not validated for case-ness

L<https://github.com/justingit/dada-mail/issues/416>
...
This release also brings back the option to enable/disable tracking clickthroughs for email address links (C<mailt:someone@example.com>), which was added in v6.3.0, and then mysteriously disappeared. 
The default is to B<disable> clickthrough tracking for these types of links. 

=head2 Bugfixes 6.7.2

=head3 "Send a Message" screen shows errors when trying to save drafts, with default backend

L<https://github.com/justingit/dada-mail/issues/410>
...
=head2 Bugfixes 6.7.1

=head3 "Unsubscriptions" option" " If an unsubscription is successful, redirect to this URL: " does not work

L<https://github.com/justingit/dada-mail/issues/405>

=head3 Dada permits illegal email addresses to be added like "user@myname. com"

L<https://github.com/justingit/dada-mail/issues/407>

=head3 ">>" characters in the button label: "Subscribe Checked Subscribers >>" cause mod_security to trigger on form submission

L<https://github.com/justingit/dada-mail/issues/408>

=head1 6.7.0

=head2 Features 6.7.0

=head3 Mass Mailing: Save as Draft
...
=head3 Mass Mailing: File Attachments
...
=head4 CAPTCHA Options

Dada Mail has shipped with support for its own CAPTCHA system, as well as support for the reCAPTCHA service (L<http://www.google.com/recaptcha>). Dada Mail also has support for the related reCAPTCHA Mailhide system (L<http://www.google.com/recaptcha/mailhide/>), to protect email addresses in its public message archives. 

The included Dada Mail Installer now allows you to configure the CAPTCHA system that comes with Dada Mail. These options are located under the, B<Advanced Configuration (Optional)> heading, and is available in our Pro Dada distribution. 

We're shipping Basic Dada Mail B<with only Amazon SES> configuration available in, B<Advanced Configuration>. We're really hoping you love Dada Mail enough to go and get a copy of Pro Dada: 

L<http://dadamailproject.com/purchase/pro.html> 

=head3 jQuery Dada Mail Subscription plugin
...
=head3 Send Mass Mailing Buttons: Always on Top

In previous versions of Dada Mail, buttons used to Test, or Send out Mass Mailing were located at the I<bottom> of the screen, after what could be a very long form, forcing you to scroll down, to submit your message.
...
=head3  Subscription Requests Approving/Denying Options
...
=head3 Subscription Process: 60+ more unit tests
...
=head3 Included CPAN Module Upgrades

=head4 CGI::Session to v4.48

=head4 GI::Session::ExpiredSessions to v1.13

=head2 Bugfixes 6.6.1

=head3 Verify Subscribers Screen shows double header

L<https://github.com/justingit/dada-mail/issues/402>
...
=item * Sending Error Detection and Reporting

=item * Redesigned Membership Admin Screen

=item * New RESTful API for Subscriptions

=back

=head2 Features 6.6.0

=head3 Mass Mailing Enhancements: Sending Error Detection and Reporting
...
=head4 Problems?! Try again.

If a message to an address is not sending out correctly, Dada Mail will now wait a small period of time (a few seconds), and try sending twice more. This should help with any I<very> temporary problems dealing with your mail system itself (your own MTA, an outside SMTP server, or service like Amazon SES). Hey, things happen, every once in awhile. The idea is that brief hiccups on your end, shouldn't mean a subscriber misses receiving an important message from you.

=head4 Still having problems? Give it a rest and try again, one more time
...
=head4 Sending Error? Log and Report.
...
Now that you know this information, you can then take some action to fix it. The majority of problems on cheap, shared hosting platforms are related to mass mailing I<speed>: you're sending too fast. 
...
Sending Errors are also reported in the right-hand table, under, B<Sending Errors> as both the total number of Sending Errors, and the percentage compared to how many addresses were sent to. 

=head4 "Sending Errors" tab

B<Sending Errors> also has its own tab, underneith, B<The Basics>. This tab will both list the addresses where Sending Errors happened, as well as a pie chart, broken down by domain. There's also an option to export these addresses as a .csv file. 

=head4 Other Tracker changes: more accurate report percentages
...
=head3 Working with Subscribers: More Powerful Tools

List Owners now have much more powerful tools to work with individual subscribers of their mailing lists, as well as the ability to work across different mailing lists. The completely rewritten Membership screen now has four separate tabs to work with your mailing list member: B<Membership>, B<Subscriber History>, B<Mailing List Activity>, B<Profile>. 

=head4 Membership

The Membership tab allows you to work with the email address of one of your mailing list members. Here, you may B<Add>, B<Update> or B<Remove> the address for your mailing list. A Dada Mail mailing list is actually composed of many different B<sublist> types. So, for example, when you send a mass mailing out, you're sending a message out to your B<Subscribers> sublist.

But, Dada Mail has other sublists that provide different roles. For example, Dada Mail's B<Black List> allows you to have a list of addresses that aren't allowed to join your list. This is also known as a, B<Suppression List>

The, B<Authorized Senders> sublist is a list of addresses that are allowed to send out a message from their own mail reader, without having to log into Dada Mail, that then gets delivered to your Subscribers (this is done with the Bridge plugin, which is bundled with Dada Mail) 
...
Dada Mail is smart when allowing you to work with the addresses that make up your mailing list, as these addresses are the most valuable asset of the whole system. Checks are done to make sure you're not going to mess up subscriptions, and all these functions only happen after Dada Mail validates your request and I<you> confirm the changes. 

Listed in the B<Membership> tab will be all the B<sublists> an individual email address is a member of, for the Mailing List you're currently logged into. Be aware that one address may be a member of I<multiple> sublists. 

For example, an address may be both a B<Subscriber> (Subscribers receive mass mailings sent to the mailing list), an B<Authorized Sender> (Authorized Senders are able to send messages via Bridge, as well as moderate messages sent via Bridge to a mailing list set up as a discussion list), as well as being on the B<White List> (Only White Listed members are allowed to subscribe to a mailing list). 

An address could also be simply on the B<Black List>, with no permission to be a member of your B<Subscribers>, until they're first removed from the Black List and added to the B<Subscribers> sublist. 

If an address is currently on the B<Subscription Requests> sublist, they will be awaiting approval from the List Owner to join the Subscribers sublist. You may B<Approve> or B<Deny> this request on this screen as well. 

=head4 Subscriber History
...
=head4 Mailing List Activity

The, B<Mailing List Activity> tab shows you how the subscriber has interacted with the mass mailings you've sent. Things like, Opens, Clickthroughs, Unsubscriptions, Sending errors, Hard/Soft Bounces are tracked for each message you send out. This information is shown for each recent mass mailing that's sent. 

Like the B<Subscriber History>, this data can also be exported in .csv format, to be opened up in an outside application. Data that's exported includes the data and time of the activity, what type of activity this was, if it was a clickthrough, what URL was clicked through, the message id of the message, as well as the IP address the subscriber was coming from. 
...
=head3 RESTful Subscription Interface EXPERIMENTAL

Dada Mail has a new B<EXPERIMENTAL> RESTful interface to its mailing list subscription system, which should allow you to use it as a service in another app of yours. For more information, see the Subscription Cookbook docs: 
...
Earlier REST and SOAP experimental client/server scripts, located in the I<dada/extras/scripts/subscribe> have now been removed. For this new RESTful interface, no additional scripts are needed to be installed. 

=head3 Administration Screen changes

Changes have been made to the, B<Administration> screen - if you are already logged into a mailing list, you'll be redirected to the default list control panel screen (usually, B<Send a Message>), rather than to the Administration screen, that simply I<tells> you you're logged in and provides a link to go this same screen. 

=head3 Black/White List Definition Changes
...
The part of an email address B<must> include the, B<@> symbol, to remove the ambiguity of if you want to Black List the user or domain part of the address and something like this doesn't happen: 

	example.com@someotherdomain.com

So the following are now I<not> valid Black List (or White List) entries: 
...
=head4 Enable Debugging Options

The included Dada Mail Installer now allows you to enabled Debugging Options for Dada Mail. These debugging options add extra log information in Dada Mail's own error log, and includes both modules that make up Dada Mail, as well as bundled CPAN Perl modules. This option is located under the, B<Advanced Configuration (Optional)> heading, and is available in our Pro Dada distribution. 

=head4 JSON module installed if needed
...
=head3 The Dada Mail Manual

The Dada Mail Manual has been updated to reflect the major overhaul fo the B<Membership> list control panel screen. 

The, B<Using Tracker> chapter has also been updated to reflect the new features in this released (logging and reporting Sending Errors)

The, B<Mailing Sending - Options> screen has also been updated to reflect the current options available. 

=head3 Profile Fields

In previous versions of Dada Mail, the app had a curious, if not downright broken behavior when it come to Profile Fields: 

If a subscriber was a member of one mailing list, and was then subscribed to another mailing list, any saved Profile Fields tied to that address would be B<lost> - even if that new subscription didn't have any new Profile Fields passed to Dada Mail.

This behavior has now changed: 

If an address, that has saved Profile Field information, and is subscribed to a mailing list, that original information will now be B<preserved>. If an address is subscribed, but had no profile field information saved, any new information would then be B<saved>. 

It's unclear yet what to do with a subscriber, that has saved Profile Field information that's subscribed to another mailing list, along with I<new> Profile Field information - should the new information be used instead? Should the old information be kept? Shoudl some sort of sophisticated folding of data happen? Should it be a pref. somewhere? (per mass import, in the list control panel?) 

We're not sure yet, but we encourage feedback. 

=head3 uncompress_dada.cgi script

The helper script named, C<uncompress_dada.cgi> is kept intentionally simple, so as to be super-quick to download, and easy to read along, if little niggling changes need to be made for your environment. We've changed a few things to make things more flexible: 

Some people moving from Basic Dada Mail, to Pro Dada mail noticed that if BOTH the C<dada-6_x_y.tar.gz> distro and C<pro_dada-6_x_y> distro are present in your installing directory, the C<dada-6_x_y.tar.gz> will be used, causing confusion. C<uncompress_dada.cgi> will now explicitly look for the C<pro_dada-6_x_y> first. 

=head2 Bugfixes 6.6.0

=head3 "Your Request For Subscription is Complete" HTML screen completely missing

L<https://github.com/justingit/dada-mail/issues/398>

=head3 ""Welcome" Message to Addresses Subscribed in the List Control Panel" does not provide unsub link, warning in the list control panel

https://github.com/justingit/dada-mail/issues/395

=head3 Mass Mailings sometime Reload, without end - browser says, "Reloading Mailing..." 

L<https://github.com/justingit/dada-mail/issues/399>
...
So, this release is out to answer that problem: instead of all the confirmation tokens being created at one time (when the temp. copy of your mailing list addresses is also made), which is what causes this slowdown, the tokens are now made right when they're needed - right before the individual message is sent out. The time to make all these tokens is most likely the I<same>, but a few minutes spread out over a few hours of sending time isn't going to be very noticeable - Dada Mail will most likely adjust its sending speeds to conpensate for any slowdown, automatically, anyways. 
...
When Bridge is run on it's cronjob schedule, it'll check for awaiting messages and send anything that's been sent to the List Email's inbox. There is a limit on how many messages Bridge will look at, in one run - and by default, it's very conservatively set to only B<1> message. If you have > 1 mailing lists using Bridge, that are both somewhat active, whatever mailing list is checked first will hog up Bridge's quota of looking at messages and your second list will never be checked. Bad news. 
...
L<http://dadamailproject.com/d/bridge.cgi.html#setup_as__mail_forward>

=head2 Bugfixes 6.5.2

=head3  Mass Mailing takes a very long time to setup, UI fails to refresh to Mailing Monitor screen. 

L<https://github.com/justingit/dada-mail/issues/394>

=head3  Bridge does not run manually/on schedule after v6.5.1 upgrade 

L<https://github.com/justingit/dada-mail/issues/393>
...
=head2 Bugfixes 6.5.1

=head3 Javascript in dada_mail.js has Internet Explorer-Specific Bugs 

L<https://github.com/justingit/dada-mail/issues/391>

=head1 6.5.0

=head2 Summary 6.5.0

=head2 Features 6.5.0
...
=item * Screen and Data Cache Control 
...
These global parameters can be read from previous configurations, and can be editing using the installer or directly in the C<.dada_config> file. 

A new doc helps explain the plethora of options available: 

L<http://dadamailproject.com/support/documentation/install_dada_mail-advanced_configuration.pod.html>

We're shipping the Installer that comes with Pro Dada with B<all these options> available to be configured. 

We're shipping Basic Dada Mail, B<with only Amazon SES> configuration available. We're really hoping you love Dada Mail enough to go and get a copy of Pro Dada: 

L<http://dadamailproject.com/purchase/pro.html> 

And honestly, we're hoping this helps to entice you to support the Dada Mail Project!
...
=head3 Plugin: Bounce Handler

=head4 Admin Menu: Bounce Scorecard Count Shown
...
=head3 JavaScript Changes

=head4 Installer-specific JavaScript: Removed from main, "dada_mail.js" File

Installer-specific JavaScript has been removed from the main, "dada_mail.js" file, creating a smaller "dada_mail.js" file. 

=head4 new, "dada_mail.user.js" file
...
The following new HTML can now be seen in the head of the C<list_template.tmpl>
...
=head3 Plugin: change_root_password.cgi DEPRECATED
...
=head2 Bugfixes 6.5.0 

=head3 Email addresses will be subscribed x times, depending on how many times they're listed in the, "sub_confirm_list" sublist

L<https://github.com/justingit/dada-mail/issues/386>

=head3  multi-byte characters in template files in dada/templates, etc are wrongly encoded/decoded when viewed 

L<https://github.com/justingit/dada-mail/issues/387>

=head3  CAPTCHA will fail, on subscription step #2, if step #1 is skipped (closed list opt in) 

L<https://github.com/justingit/dada-mail/issues/385>

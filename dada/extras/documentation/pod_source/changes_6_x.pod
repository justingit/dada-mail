...
=head3 Core5 Labs FileManager Support

Dada Mail now supports the Core5 FileManager, along with KCFinder. More information on FileManager: 

L<http://labs.corefive.com/projects/filemanager/>

Learn more about WYSIWYG Editors and File Browser support in Dada Mail: 

L<http://dadamailproject.com/d/features-wysiwyg_editors.pod.html>

We've added FileManager support to Dada Mail, because some people are having trouble with KCFinder's php backend code. 

FileManager's backend connector (that we ship) is written in Perl, so if you can run Dada Mail, using FileManager should work equally well. 

=head3 CKEditor Upgraded to v4.3

CKEditor has been upgraded from v3.x to v4.3

=head3 CKEditor: Template Tag Dropdown Menu

The CKEditor now has a dropdown menu allowing you to easily access some the most useful and familiar email template tags, Subscriber Profile Fields, List Settings and control structions (loop, if, unless, etc). Select a tag to have it placed into your editing window. 

=head3 TinyMCE Upgraded to v4.0.11

TinyMCE has been upgraded from v3.x to v4.0.11

=head3 Bridge: Moderators and Authorized Senders

The job of the Authorized Senders sublist has been split into two sublists: 

B<Authorized Senders> will continue to be the sublist of addresses, that may post to a mailing list, without having to be a member of the Subscribers. 

B<Moderators> will now simply be a sublist of addresses that get sent out a confirmation email to approve or deny message postings

=head3 Bridge: "On Behalf Of"; (p.p. mode) Support

Discussion Messages may now be sent, "On Behalf Of" the original sender by the List Owner. This allows you to set up a discussion list with Amazon SES, which only allows messages from verified senders. You may now verify only your List Owner, and still have a discussion list. 

=head3 Mass Mailing Logs: Saveable, viewable in the Log Viewer Plugin

Logs made for specific mass mailings used to be deleted, when a mass mailing was finished, they're now saved, so that you may view them at a later time, using the Log Viewer plugin. 

=head2 Change 6.8.0

=head3 Amazon SES Auto-Adjust Batch Sending Rates

Dada Mail has an option (which we think you should use) to automatically adjust batch sending rates, when sending with Amazon SES. We've modified the algorithm that figures out this rate to be more aggressive with sending speed, without going over your spec'd Amazon SES sending limits. Welcome to faster sending w/Amazon SES! 

=head3 FCKeditor Support Dropped

With the upgrade of CKeditor to v4.x, we've now fully dropped support for FCKeditor. FCKeditor has been listed as, "deprecated" for over a year and during that time, it has been not been advised to be installed at all. 

...
=head2 Summary 6.4.0 Beta 1

=head2 Features 6.4.0 Beta 1

=head3 Unsubscription Process: Vastly Simplified

The unsubscription process for Dada Mail has been redesigned, and has been vastly simplified. 

All mass mailings require an unsubscription link to be present in the message itself. This unsubscription link is now unique to the list, mass mailing and subscriber it's sent to. 

Clicking the unsubscription link will bring the subscriber to a screen, with a simple form, which only requires that they fill out their email address. An optional email address hint can be shown, to help subscribers know which email address of they're subscribed with. Once a subscriber has filled out their email address and submitted the form, they will be unsubscribed immediately. 

This unsubscription link will be removed in discussion lists, so replies will not potentially re-send the unsubscription link of a subscriber, to the entire mailing list. Unsubscription links will also be removed from archive messages. 

To make the unsubscription process as simple and fool-proof as possible, many of the advanced options that were available in the mailing list's control panel have been removed. This was done to make 100% sure that users of Dada Mail were following email best practices, as well as following the newest revision of the CAN SPAM 2008 revision. 

Unsubscriptions are now tracked, and data can be viewed/exported in the Tracker plugin. The date of the unsubscription, IP address, subscriber email address, mass mailing the unsubscription was made from, and the mailing list name will be saved.
...
Tracker preferences have been simplified, to make sure as much data is collected by default. You may now track clickthroughs automatically, or manually; track message opens directly or indirectly and track with or without your email data. 
...
You may now search for more than one term, per field, at one time - example: a First Name of either I<Justin>, OR I<John>. 
...
=head2 Bugfixes 6.3.1

=head3 Sending Subscription Approval/Denial messages causes server error

L<https://github.com/justingit/dada-mail/issues/370>

...
WYSIWYG Editors and File Browsers in Dada Mail: 

L<http://dadamailproject.com/d/features-wysiwyg_editors.pod.html>

We've added FileManager support to Dada Mail, as people were having trouble with KCFinder's php code. FileManager's connector (that we ship) is written in Perl, so if you can run Dada Mail, using FileManager should work equally well. 
...
With the upgrade of CKeditor, we've now dropped support fully for FCKeditor. FCKeditor has been listed as, "Deprecated" for over a year and during that time, it has been not been advised to be installed at all. 

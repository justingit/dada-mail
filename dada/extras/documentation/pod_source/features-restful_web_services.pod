...
Both the public, and private key for your mailing list can be accessed in the mailing list's control panel under, 

I<Your Mailing List - Web Services API>
...
The Perl Client is called, C<DadaMailWebService.pm> and is located at I<dada/extras/scripts/webservices/DadaMailWebService.pm>.

=head4 Synopsis

    # Change this to path of the following module: 
    use lib qw(/path/to/webservices/module); 
    ...
    my $list        = 'example';
    my $public_key  = 'aaOKqoeobNoaJPyAMYHo';                        
    my $private_key = 'bju7EF35jHua5aYvdiEyboipDcBQo3eeRqCNfc4E';
...
=head4 Synopsis: 
    ...
    $list        = 'example';
    $public_key  = 'aaOKqoeobNoaJPyAMYHo';                        
    $private_key = 'bju7EF35jHua5aYvdiEyboipDcBQo3eeRqCNfc4E';

    $ws = DadaMailWebService->new(
        $server,
        $public_key,
        $private_key
    );
...
Depending on the web service called, the payload is just HTTP POST variables - all web services for Dada Mail only support using the POST method. The paramaters in the Payload must be in alphabetical order, so that the digest created on both the client and server match. 

=head3 Services

=head4 validate_subscription

C<validate_subscription> takes a list of addresses (and associated metadata) and validates the subscription, but does not subscribe any of the addresses. 

...
Once enabled, this feature will make sure the process of sending out a mailing will succeed to completion. 
...
If you're using the Dada Bridge plugin, it also has the same functionality as the B<Mailing Monitor> plugin built in (although, no report will ever be printed out). Because of this, you won't be absolutely required to run both, but if you want to run both, stagger the timing of the cronjobs - if you're running Dada Bridge every 5 minutes, run the B<Mailing Monitor> plugin every 6. 

=head2 I'm a nerd. Any documentation on the Mass Mailings/Auto Pickup API? 
...
L<http://dadamailproject.com/d/Config.pm.html#_program_error_log>

=item * Setup Debugging for various CPAN modules

If you're using SMTP sending, set up the debugging/trace settings for Net::SMTP and Net::POP3. This is done in the Config.pm file, in the variable, C<%CPAN_DEBUG_SETTINGS>. More information: 

...
The same thing as a Mass Mailing. These can be used interchangably. We do prefer, "Mass Mailing" 
...
Read below on how to set up a cronjob (scheduled task) to monitor your mailing for you. 
...
Batch Sending is used primarily to I<slow down> a mass mailing and I<space out message sending> over a longer period of time. It has basically two parts to it: 
...
Batch setting is configured in the list's control panel: Log into your list and go to: B<Mail Sending - Mass Mailing Preferences>. 


=head2 Should I send my mailing in batches? 
...
=item * Hourly Email Sending Limitations
...
You can configure the feature in your List Control Panel, under B<Mail Sending - Mass Mailing Preferences>. 

Unlike traditional Mailing List Managers (Mailman, Majordomo, EZelm), B<Dada Mail does not run as a daemon> - (and this is simplifying things) which is a program that is B<always running>, just waiting to fill out requests. 

Shared server environments usually have a set limit on the amount of memory a program process can consume, how large the process itself can be, how many files it can have open and most importantly for us, B<how long a process can run for>. It can be a restrictive environment. 
...
=head2 During a Mass Mailing, is it safe to navigate away from the Sending Monitor screen? 
...
=head3 The, Mailing Monitor Plugin

B<See the Mailing Monitor Plugin Documentation:>

L<http://dadamailproject.com/d/mailing_monitor.cgi.html>

Which can be run via cronjob. If this plugin is installed and running correctly, you won't have to use the B<Monitor Your Mailings> screen at all.

If you do use the, I<Restart Mailings After Each Batch>, this plugin isn't really going to work very well, since you're probably going to run it via cron much less than is realistically needed for your mailing to go out in good time. 
...
When set to, B<1> after every time Dada Mail is run via the, C<mail.cgi> script, the mailing monitor will be run, exactly like the C<Mailing Monitor> plugin and monitor your mailings. This is somewhat of an imperfect option for you, but could come in handy if you cannot or do not know how to set up a cronjob. 
...
If you're using the Dada Bridge plugin, it also has the same functionality as the B<Mailing Monitor> plugin built in. 

=head2 Mass Mailings/Auto Pickup API 

See: L<http://dadamailproject.com/support/documentation/Mail_MailOut.pm.html>
...
=head1 Multiple Mass Mailings at the Same Time

Multiple Mass Mailings can go on at one time. 

By default (conservatively) Dada Mail only allows you to have B<one> mailout at one time. You can change this in your config using the variable, C<$MAILOUT_AT_ONCE_LIMIT>.
...
Any mailouts that have been submitted to Dada Mail that are over the limit set in C<$MAILOUT_AT_ONCE_LIMIT> will be B<queued>. They'll just wait in line until the number of mailouts is less than the limit. 
...
According to L<http://helpdesk.bluehost.com/index.php/kb/article/000469> 

...
Read below on how to set up a cronjob (scheduled task) to monitor your mailing for you, so you don't have to keep this window open.  
...
L<http://dadamailproject.com/pro_dada/mass_mailing_preferences.html>
...
=head2 The Mailing Monitor Plugin

The B<Mailing Monitor> plugin, which comes with Dada Mail is designed to run on a schedule, and monitor any outgoing mass mailings. If it detects a mass mailing has stopped sending, it will safely restartt the mass mailing at the point it left off. 

Setting it up correctly means that you B<do not have to keep the Monitor Your Mailings> screen open - monitoring of your mass mailings will happen in the background. 

B<See the Mailing Monitor Plugin Documentation:>

L<http://dadamailproject.com/d/mailing_monitor.cgi.html>

This plugin is designed to be run via cronjob, on a repeating schedule (say, every 5 minutes). 

=head3 Alternatives to the Mailing Monitor Plugin

We highly suggest the Mailing Monitor plugin, but other options are present - you may use any of these options with each other. 


=head4 The Monitor Your Mailings Screen 
...
Keeping this screen open until your mass mailing has finished will make sure your mass mailing goes to completion. If you haven't set up the cronjob for the Mailing Monitor B<plugin> this is the best way to keep things running smoothly. 
...
=head4 Setting, $MONITOR_MAILOUTS_AFTER_EVERY_EXECUTION to, 1

Another option, is to set the global config variable, C<$MONITOR_MAILOUTS_AFTER_EVERY_EXECUTION> to, B<1>. 
...
L<http://dadamailproject.com/support/documentation-6_8_1/features-amazon_ses_support.pod.html>
...
L<http://docs.cpanel.net/twiki/bin/view/AllDocumentation/WHMDocs/MailLimitingFeatures>

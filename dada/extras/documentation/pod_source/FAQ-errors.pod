...
=head1 In-Browser Error Messages
...
Dada Mail's, C<mail.cgi> script, as well as all of its plugins and extensions that you are utilizing will need to have their file permissions changed so that they can be executed, when you visit them in your web browser. This usually means changing the file permissions to, B<755>. 
...
You'll find a link for the Log View plugin on the left hand menu, under: B<Plugins - Log Viewer>. Once on the Log Viewer screen, Find the popup menu labeled, B<View Log:> and change it to, B<Error Log>. The Log Viewer should refresh and show the last few lines of your Dada Mail error log. 
...
If you do post a question about an error to the Dada Mail support boards or dadadev mailing list, the first thing you're usually going to be asked is error log snippets, as well as the B<Version of Dada Mail> you are running. 
...
=head1 Advanced Error Logging and Reporting
...
B<http://dadamailproject.com/support/documentation/global_variables.pod.html#_cpan_debug_settings>
...
B<See the FAQ on Mass Mail Sending:> L<FAQ-mailing_list_sending.pod.html>
...
=head1 Diagnosing Error Messages
...
See, B<How to install CPAN modules>: L<http://dadamailproject.com/d/FAQ-general.pod.html#how_do_you_install__cpan_perl_modules>

Dada Mail also ships with a disabled copy of B<JSON>. In the I<dada/DADA/perllib> directory, find the directory named, 

C<JSON-remove_to_install>

and the file named, 

C<JSON.pm-remove_to_install> 
...
=head3 Byte order is not compatible

The error will look similar to this, 

	[Sat Sep 1 02:19:42 2012] scheduled_mailings.pl: Byte order is not compatible at /usr/local/lib/perl5/5.8.8/x86_64-linux/Storable.pm line 417, at ../DADA/perllib/MLDBM/Serializer/Storable.pm line 27

Try restoring your Schedules, by using going to the Restore Mailing Lists screen in Dada Mail - ie: 

L<http://example.com/cgi-bin/dada/mail.cgi?f=restore_lists> 

You will only need to restore your mailing list Schedules. 
...
http://dadamailproject.com/support/documentation-4_0_2/global_variables.pod.html#_template_settings

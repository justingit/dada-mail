...
=head1 Dada Mail and Google Apps for Domains
...
=head2 Using Dada Bridge for Discussion Lists

=head3 Method #1 Setup the List Email account on Google Apps, use the Google Apps login info

You'll probably be able to use Dada Bridge with Google Apps, since Dada Bridge only needs to be able to I<receive> email, if like above, 
...
for this List Email into Dada Bridge. A few things should be the same for all Google Apps email accounts, as is listed at: 
...
=head2 Using Mystery Girl (Bounce Handler)
...
but it I<can> be used locally by your local mail server to store the bounce messages, as well as by Mystery Girl to, I<check> this local messages. 

=head3 Method #2 Setup a subdomain with a different MX record

Just like with Dada Bridge, another option would be just to set up a new subdomain, with a different MX record that points the subdomain's email delivery to your hosting account's mail server, instead of Google Apps. 
...
Will simply write out all email messages sent from all mailing lists (that are set up to use sendmail to send out messages) into a file in the C<.dada_files/.tmp> directory called, C<mail.txt>

You can then manually inspect the output of what Dada Mail is creating to spot any problems. 

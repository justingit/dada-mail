...
Originally, if the StopForumSpam service was down, any address given to it to be checked would return a 0 status, meaning that the address is listed as problematic in StopForumSpam's database. This could lead to a lot of false negatives, which is absurd behavior.
...
We've forked the version of StopFormSpam that's available on CPAN, which reflects this behavior. That version can be found here: 

...
One frustration of administrating a large collection of mailing lists with Dada Mail 
is that each mailing list must be administrated one at a time, so it was not possible to make
changes to mailing list settings fo all mailig lists at once. 

With v9.1.0, we've introduced the ability to saving the List Settings to multiple mailing lists at once. 
...
You will need to be logged into a mailing list with the Dada Root Password, otherwise the feature will not be available. 
Look for the button labeled, B<Save For Multiple Lists...> When clicked, a modal window will open, allowing you to select the mailing lists that you would like to save the list settings to. 

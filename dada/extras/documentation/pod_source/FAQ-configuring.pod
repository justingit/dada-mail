...
We do highly suggest that you do not edit the C<Config.pm> directly, unless you need to (for, whatever reason), but use The Advanced Installation Method when you do make a new installation of Dada Mail: 
...
If these variables are not customized for your specific installation, more likely than not, you're using the Advanced Installation Method and have your global configuration saved in the, C<.dada_config> file. 
...
Methods, several oft-used global configuration variables are placed in the C<.dada_config> file, but the are not active. To activate them, you must remove the C<=cut> strings that are placed right before and right after the chunk of variables they're inactivating. For example:

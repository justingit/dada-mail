...
Digest messages are messages that are created by taking the content of many messages sent in a timespan. See the feature overview of Digests, here: 

L<http://dadamailproject.com/d/features-digest.pod.html>

Subscribers can now have the option of having individual messages delivered, a digest message delivered, or having message held from delivery.

The digest message sent out (Subject, PlainText version and HTML version)  can be fully customized. 

Way back in the day, Dada Mail had Digest support - it totall sucked. This new support totally doesn't.

=head3 Scheduled Mass Mailings

Dada Mail has had support for scheduled mailings since the inception of the Beatitude Plugin. That plugin has been marked as B<deprecated>, and we've taken one giant step in integrating scheduling of mass mailings into the core of Dada Mail with this release. 

Now, any draft message can be scheduled to be sent at a later time, from within the Send a Message or Send a Webpage screens. There's no need to switch to the plugin to schedule a mailing.

We'll still ship with the Beatitude plugin, but it's days are numbered. 

To create a new Schedule, first go to the Send a Message, or Send a Webpage screen. Write your message! Click the, "Schedule..." button. Tick, "Activate" and set the time in the future you would like your message to go out. Click, "Create Schedule" and, you're done!

Make sure the Mailing Monitor's cronob is also set up. (I<It should already be to make sure mass mailings go smoothly.>)

Once a message has been scheduled, you can edit the schedule (and even reuse the schedule, if you would like!). In the list control panel, Go to: i<Mass Mailing - Drafts/Stationary/Schedules>. Click the B<Schedules> tab to view all your schedules. 
...
Similar to Message Drafts, Dada Mail now supports B<Stationary>. 
Stationary are messages you can use as a starter for mailing list messages you would like to send out. 
Unlike a draft message, Stationary will be saved to be reused for as many times as you'd like. 
...
The B<default_mass_mailing_messages.cgi> plugin has similar features to the Stationary feature. 

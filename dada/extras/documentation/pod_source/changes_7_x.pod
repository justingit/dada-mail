...
This is primarily a bugfix release, we have added a few new options available in the included Dada Mail Installer under, B<Advanced Configuration (Optional)> including enabled Global Blacklist and Global Unsubscribe support, as well as the ability to set the percentage of your sending quota that you allow Dada Mail to use, for Amazon SES sending.

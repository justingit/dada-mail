...
Here's the full docs of our new Experimental Web Services API: 

L<http://dadamailproject.com/d/features-restful_web_services.pod.html>

In a nutshell, these new webservices give you administrative power to validate and subscribe addresses, unsubscribe addresses get and set list settings and send out mass mailings. 
...
This complaint can be seen in the Tracker plugin's report, and an email will be sent to the List Owner, allowing them to know about the complaint, as well as giving them an easy way to remove the address for their mailing list. 

=head3 Amazon SES: Warnings about Unverified Senders

One of the problems of Amazon SES sending is that if the List Owner, and List Administrator are not verified manually to send via Amazon SES, whole mass mailings can fail.

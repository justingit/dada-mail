...
=head1 Subscriber Profile Fields in Dada Mail 3
...
Like many of the features in Dada Mail, we've tried to create a good balance of dead-simple to use and highly configurable. The Subscriber Profile Fields project is a very large feature with a lot of interesting ways we can take it. We're starting out simple and easy, but we'd like to know what certain features are important to you - so let us know via our email, the dadadev mailing list and the support boards (L<http://dadamailproject.com/support/boards>)

=head1 Implementation

The extra Subscriber Profile Fields in Dada Mail are saved in the C<dada_subscribers> table. Right now, there's a limit of 100 extra columns and each column is given the data type of, C<text>. 
...
Adding a new subscriber field is pretty easy. Log into the list control panel B<with the Dada Mail Root Password> (very important) to any list. On the left hand menu, under, B<Manage Subscribers>, click, B<Subscriber Profile Fields>
...
Please note that removing any Subscriber Profile Fields will B<permanently delete all the information that once was saved in this particular field>. 
...
The other thing to note is that Subscriber Profile Fields are B<global>, meaning, they'll be there for each list Dada Mail manages. 
...
Dada Mail currently doesn't support changing the order of these Subscriber Profile Fields, but if you feel mighty, you may do this yourself via SQL. If you had two Subscriber Profile Fields, one called, "last_name" and one called, "first_name" and you'd like "first_name" come before, "last_name", you'd want to say do something like: 
...
Currently, this is done in the, "Send a Message" and, "Send a Webpage" screens. Click the, B< Show/Hide Partial Sending Options>. You'll be presented with a form that allows you to make simple queries and give you a preview of who's going to the message. 
